    ...
    <el-card class="search-card">
      <el-row :gutter="20">
        ...
        <el-col :xs="24" :sm="12" :md="4">
          <el-select
            v-model="queryParams.isShared"
            placeholder="共享状态"
            clearable
            style="width: 100%"
            @change="handleSearch"
          >
            <el-option label="仅共享" :value="1" />
            <el-option label="非共享" :value="0" />
          </el-select>
        </el-col>
      </el-row>
              ...
              >
                <div v-loading="fileUploading">
                  ...
                  <div class="el-upload__tip">
                    {{ resourceForm.resourceType ? `支持格式：${getAcceptFileTypes()}` : '请先选择资源类型' }}
                  </div>
                </div>
              </el-upload>
              <el-input
...
import { Search, Plus, Delete, Refresh, Picture, Upload, Link } from '@element-plus/icons-vue'
import {
  getResourcePage,
  getResourceById,
  createResource,
...
const useUrlMode = ref(true) // true: URL模式, false: 上传模式
...
const handleSubmit = async () => {
  ...
  // 额外验证：确保资源URL已设置（无论是通过URL输入还是文件上传）
...
const handleDelete = async (resourceId) => {
  try {
    await ElMessageBox.confirm('确定删除该资源吗？此操作不可恢复！', '警告', {
      type: 'warning',
...
const handleBatchDelete = async () => {
  try {
    await ElMessageBox.confirm(
      `确定删除选中的 ${selectedIds.value.length} 个资源吗？此操作不可恢复！`,
      '警告',
      {
        type: 'warning',
        confirmButtonText: '确定',
    ...
    await batchDeleteResource(selectedIds.value)
...
const handleOffline = async (row) => {
  try {
    await ElMessageBox.confirm('确定下架该资源吗？', '提示', {
      type: 'warning',
      confirmButtonText: '确定',
      cancelButtonText: '取消'
    })
...
const handleShareChange = async (row) => {
  try {
    await updateResourceShare(row.id, row.isShared)
    ElMessage.success(row.isShared === 1 ? '已设为共享' : '已取消共享')
  } catch (error) {
    // 恢复原状态
    row.isShared = row.isShared === 1 ? 0 : 1
